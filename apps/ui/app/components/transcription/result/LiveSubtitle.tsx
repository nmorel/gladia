...
export function parseSrtTime(time: string) {
...
export function getLiveSubtitle(
  ...
  const {transcription, speaker} =
    parsedValue?.find(({time_begin, time_end}) => {
      return time_begin <= currentTime && time_end >= currentTime
    }) ?? {}
...
export function LiveSubtitle({
  ...
  return <span>{getLiveSubtitle(data, property, timestamp)}</span>
}
